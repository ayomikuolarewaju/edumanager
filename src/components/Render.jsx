...
import {updateCourse} from '../actions/action.js'

const Render = () => {
     const [data, setData] = useState([])
     const [edtData,setEdtData] = useState({})
     const [isVisible,setIsVisible] = useState(true)
     const [pending,formAction,message] = useActionState(updateCourse,null)
        ...
        const makeVisible =async (id) =>{
              ...
              const resp = await axios.get(`http://127.0.0.1:8080/api/v1/students/course/${id}`);
                    ...
                    </thead>
                    {
                        ...
                        <tr className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-3">
                                ...
                                <button onClick={()=>{ 
                                    axios.delete(`http://127.0.0.1:8080/api/v1/students/course/delete/${d.id}`) 
                                    redirect('/')}
                            ...
                            <div className='h-[700px] w-[900px] bg-white shadow-md shadow-black flex flex-col justify-center items-center '>
                                  ...
                                  <div className='absolute top-[120px] font-bold text-2xl'><h4>Edit Course</h4></div>
                                    <Formsy clicker={formAction}
                                       ...
                                       div2={!true} name2='description' type2='text' label2='Describe the course'  value2={edtData.description}
                                       div3={!true} name3='starttime' type3='date'   label3='Commencement Date' value3={edtData.starttime}
                                       okaybtn={!true} action='Edit Course'/>
                                 </div>
                                 </div>
            }
